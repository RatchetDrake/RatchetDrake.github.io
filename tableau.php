<!DOCTYPE html>
<html>
<head>
    <title>Tableau</title>
    <link rel="stylesheet" href="style3.css">
</head>
<body>
    <div>
    <table border="1">
        <tr>
            <td class="vert" colspan="7">Jeux Olympiques de Rio 2016</td>
        </tr>
        
            <td class="noire" colspan="3">Pays</td>
            <td class="noire" ><img src="./image/or.png"></td>
            <td class="noire"><img src="./image/argent.png"></td>
            <td class="noire"><img src="./image/bronze.png"></td>
            <td class="noire"><img src="./image/total.png"></td>
           
        <tr>
            <td class="noire">1</td>
            <td class="npays"colspan="2"><img src="./image/usa.png"> USA</td>
            <td class="blanc" >46</td>
            <td class="blanc">37</td>
            <td class="blanc">38</td>
            <td class="jaune">121</td>
           
            
            
        </tr>
        <tr>
            <td class="noire">2</td>
            <td  class="npays" colspan="2"><img src="./image/grande_bretagne.png"> Grande-Bretagne</td>
            <td class="blanc" >27</td>
            <td class="blanc">23</td>
            <td class="blanc">17</td>
            <td class="jaune" >67</td>
        </tr>
        <tr>
            <td class="noire">3</td>
            <td class="npays" colspan="2"><img src="./image/chine.png"> Chine</td>
            <td class="blanc">26</td>
            <td class="blanc">18</td>
            <td class="blanc">26</td>
            <td class="jaune">70</td>
        </tr>
        <tr>
            <td class="noire" >4</td>
            <td class="npays" colspan="2"><img src="./image/russie.png"> Russie</td>
            <td class="blanc">19</td>
            <td class="blanc">18</td>
            <td class="blanc">19</td>
            <td class="jaune">56</td>
        </tr>
        <tr>
            <td class="noire">5</td>
            <td class="npays" colspan="2"><img src="./image/allemagne.png"> Allemagne</td>
            <td class="blanc">17</td>
            <td class="blanc">10</td>
            <td class="blanc">15</td>
            <td class="jaune">42</td>
        </tr>
        <tr>
            <td class="noire">6</td>
            <td  class="npays"colspan="2"><img src="./image/japon.png"> Japon</td>
            <td class="blanc">12</td>
            <td class="blanc">8</td>
            <td class="blanc">21</td>
            <td class="jaune">41</td>
        </tr>
        <tr>
            <td class="noire">7</td>
            <td class="npays" colspan="2"><img src="./image/france.png"> France</td>
        
            <td class="blanc">10</td>
            <td class="blanc">18</td>
            <td class="blanc">14</td>
            <td class="jaune">42</td>
        </tr>
        <tr>
            <td class="vert" colspan="7">Ce classement est limité aux 7 premières nations</td>
        </tr>
    </table>
    </div>
</body>
</html>
